(() => {
  ...
  window.LC_PLAYER_DIAGNOSTICS = () => ({
    ...
    inlineTrial: { ...inlineTrial.diagnostics },
    topic: state.topic,
  ...
  // EXPERIMENT ONLY: keep the original outer frame dimensions and all page UI.
  // A larger inner browsing viewport may avoid oversized/clipped Drive controls.
  // 640x360 is a trial reference size, NOT a documented Google minimum.
  // This does not read/modify Google's DOM, cookies, permissions, or video data.
  const inlineTrial = {
    sequence: 0,
    raf: 0,
    observer: null,
    ...
    diagnostics: { mode: "not-opened", providerPlayback: "unobservable-cross-origin" }
  };

  function restoreInlineFrame() {
    const frame = inlineTrial.savedFrame;
    const attrs = inlineTrial.savedAttributes;
    if (!frame || !attrs) return;
    Object.keys(attrs).forEach(name => {
      if (attrs[name] === null) frame.removeAttribute(name);
      else frame.setAttribute(name, attrs[name]);
    });
  }

  function stopInlineTrial() {
    inlineTrial.sequence += 1;
    if (inlineTrial.raf) cancelAnimationFrame(inlineTrial.raf);
    inlineTrial.raf = 0;
    if (inlineTrial.observer) inlineTrial.observer.disconnect();
    inlineTrial.observer = null;
    restoreInlineFrame();
    inlineTrial.savedFrame = null;
    inlineTrial.savedAttributes = null;
    inlineTrial.diagnostics = {
      mode: "closed", providerPlayback: "unobservable-cross-origin"
    };
  }

  function fitInlineViewport() {
    const frame = inlineTrial.savedFrame;
    const box = frame && frame.parentElement;
    if (!frame || !box || $("videoModal")?.classList.contains("hidden")) return false;
    const activeFull = document.fullscreenElement || document.webkitFullscreenElement;
    if (activeFull && (activeFull === box || activeFull.contains(frame))) {
      restoreInlineFrame();
      inlineTrial.diagnostics.mode = "native-fullscreen-original-sizing";
      return true;
    }
    const css = getComputedStyle(box);
    const px = value => Number.parseFloat(value) || 0;
    const rect = box.getBoundingClientRect();
    const width = rect.width - px(css.borderLeftWidth) - px(css.borderRightWidth);
    const height = rect.height - px(css.borderTopWidth) - px(css.borderBottomWidth);
    if (!(width > 0 && height > 0)) return false;

    const scale = Math.min(1, width / 640, height / 360);
    ...
    // Explicit viewport attributes supplement CSS for embedded WebViews.
    frame.setAttribute("width", String(Math.round(innerWidth)));
    frame.setAttribute("height", String(Math.round(innerHeight)));
    inlineTrial.diagnostics = {
      mode: scale < 1 ? "scaled-inner-viewport" : "pixel-sized-inner-viewport",
      frameWidth: Math.round(width), frameHeight: Math.round(height),
      innerWidth: Math.round(innerWidth), innerHeight: Math.round(innerHeight),
      scale: Number(scale.toFixed(4)),
      providerPlayback: "unobservable-cross-origin"
    };
    return true;
  }

  function scheduleInlineFit() {
    if (inlineTrial.raf) cancelAnimationFrame(inlineTrial.raf);
    inlineTrial.raf = requestAnimationFrame(() => {
      inlineTrial.raf = 0;
      fitInlineViewport();
    });
  }

  function loadInlinePreview(frame, preview) {
    stopInlineTrial();
    inlineTrial.savedFrame = frame;
    inlineTrial.savedAttributes = {};
    ["style", "width", "height"].forEach(name => {
      inlineTrial.savedAttributes[name] = frame.getAttribute(name);
    });
    const sequence = inlineTrial.sequence;
    // Existing openVideo() unhides the existing modal synchronously afterwards.
    // Wait for its measured frame rather than loading into a hidden viewport.
    let attempts = 0;
    const mount = () => {
      inlineTrial.raf = 0;
      if (sequence !== inlineTrial.sequence || $("videoModal")?.classList.contains("hidden")) return;
      if (!fitInlineViewport() && attempts++ < 5) {
        inlineTrial.raf = requestAnimationFrame(mount);
        return;
      }
      // Geometry changes never reassign src after this initial navigation.
      if (frame.getAttribute("src") !== preview) frame.src = preview;
      if (typeof ResizeObserver === "function") {
        inlineTrial.observer = new ResizeObserver(scheduleInlineFit);
        inlineTrial.observer.observe(frame.parentElement);
      }
    };
    inlineTrial.raf = requestAnimationFrame(mount);
  }

  window.addEventListener("resize", scheduleInlineFit);
  document.addEventListener("fullscreenchange", scheduleInlineFit);
  document.addEventListener("webkitfullscreenchange", scheduleInlineFit);
  ...
  function openVideo(videoId) {
    ...
    if (player) {
      ...
      loadInlinePreview(player, preview);
    }
